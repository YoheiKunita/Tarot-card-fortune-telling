// AdviserService: stable facade for generating tarot advice/readings
// CommonJS module for use from Electron preload.
...
class AdviserService {
  constructor(opts = {}) {
    this.opts = Object.assign({
      ...
      cache: null,
      adviserExports: null, // optional injection of existing module { createClient, GenerateReadingUseCase }
    }, opts);

    // Try lazy-load tarot-adviser if not injected
    if (!this.opts.adviserExports) {
      try {
        // Expect directory with index.js under lib/tarot-adviser
        // Resolve relative to project root (where preload.js resides)
        const mod = require(path.join(__dirname, 'tarot-adviser'));
        this.opts.adviserExports = mod;
      } catch (_) {}
    }
  }
  ...
  async generate(input = {}) {
    ...
    const backend = this.#pickBackend(normalized);
    ...
    // cache key (optional)
    const key = this.#cacheKey(normalized, backend);
    ...
    if (backend === 'openai' && this.#canUseOpenAI()) {
      try {
        const run = () => this.#runOpenAI(normalized);
        const res = await this.#retryWithTimeout(run);
        result = this.#ok(this.#mapUsecaseResult(res, normalized.locale), inferenceId, 'openai', t0);
      } catch (e) {
        this.#log('warn', '[AdviserService] OpenAI error, fallback to stub:', this.#safeMsg(e));
        const fb = this.#stubResult(normalized);
        result = this.#ok(fb, inferenceId, 'stub', t0, 'OPENAI_ERROR');
      }
    } else {
      const fb = this.#stubResult(normalized);
      result = this.#ok(fb, inferenceId, 'stub', t0, backend === 'openai' ? 'OPENAI_NOT_AVAILABLE' : null);
    }
  ...
  async #runOpenAI(n) {
    ...
    return await usecase.execute({
      question: n.question || '',
      cards: n.cards || [],
      spread: n.spread || null,
      userId: n.userId || null,
    });
  }

  #mapUsecaseResult(res, locale) {
    // Assume res.reading.summary exists; otherwise provide minimal mapping
    const reading = res && res.reading ? res.reading : null;
    if (reading) return { reading, valid: true, meta: {} };
    // graceful fallback mapping
    return {
      reading: {
        summary: locale === 'ja' ? '結果を取得しました。' : 'Result retrieved.',
        cards: [],
      },
      valid: true,
      meta: {},
  ...
  #safeMsg(e) { return (e && e.message) ? e.message : String(e); }
  ...
  #cacheGet(key) {
    ...
    try { return this.opts.cache.get(key) || null; } catch { return null; }
  }

  #cacheSet(key, val) {
    if (!this.opts.cache) return;
    try { this.opts.cache.set(key, val, 60_000); } catch {}
  }
}

...
class ConsoleLogger {
  info(msg, meta) { try { console.log("[info]", msg, meta || ""); } catch (_) {} }
  warn(msg, meta) { try { console.warn("[warn]", msg, meta || ""); } catch (_) {} }
}

class GenerateReadingUseCase {
  /**
   * @param {Object} deps
   * @param {import('./llmClient').LLMClient} deps.llmClient
   * @param {{get:Function,set:Function}=} deps.cache
   * @param {{info:Function,warn:Function}=} deps.logger
   */
  constructor({ llmClient, cache, logger }) {
    this.llm = llmClient;
